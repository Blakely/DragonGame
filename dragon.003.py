...
def randomizeCaves():
	...
	for x in range(0,2):
		randompathMap[str(x)]=paths.pop(randompathMaps())
		for i in range(0,2):
			randompathMap[str(x)+str(i)]=paths.pop(randompathMaps())
			for b in range(0,2):
				randompathMap[str(x)+str(i)+str(b)]=outcomes.pop(randomOutcomes())
	
	return randompathMap
...
def main():
	...
	resetP = ["There is a fork in the path - left or right (0 or 1)?",
	...
	resetO = ["ANND you have a Heart attack from the suspense.",
